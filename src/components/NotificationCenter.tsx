...
export default function NotificationCenter({notifications,onRead,onReadAll}:Props){
 ...
 return <section className="panel notificationCenter">
  <div className="heading"><div><h2>Notifications</h2><p>{unread} unread today</p></div>{unread>0&&<button onClick={onReadAll}>Mark all read</button>}</div>
  {notifications.length===0?<div className="message">You're all caught up for today.</div>:<div className="notificationList">
