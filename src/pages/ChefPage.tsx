...
export default function ChefPage({user,profile}:Props){
 ...
 <nav className="workspaceNav">{[["dashboard","🧑‍🍳 Dashboard"],["menu","🍽️ Menu"],["alerts","⚠️ Allergy Alerts"],["headcount","👥 Headcounts"],["late","🥡 Late Plates"],["print","🖨️ Print"]].map(([k,l])=><button key={k} className={tab===k?"active":""} onClick={()=>setTab(k)}>{l}{k==="alerts"&&conflicts.length?` (${conflicts.length})`:""}</button>)}</nav>{error&&<div className="error">{error}</div>}
