...
export default function AdminPage({user,profile}:Props){
 ...
 const [preview,setPreview]=useState<"member"|"chef"|null>(null),[roleNotice,setRoleNotice]=useState("");
 async function load(){setLoading(true);setError("");const [p,m]=await Promise.all([supabase.from("profiles").select(PROFILE_COLUMNS).order("full_name",{nullsFirst:false}),supabase.from("meals").select(MEAL_COLUMNS).order("meal_date").limit(100)]);setError(firstErrorMessage(p.error,m.error));setMembers(p.data||[]);setMeals(m.data||[]);setLoading(false)}
 useEffect(()=>{load()},[]);
 ...
 {tab==="roles"&&<div className="adminPanel panel"><h2>Roles</h2><p>Role changes are protected by Supabase permissions.</p>{roleNotice&&<div className="message"role="status">{roleNotice}</div>}<div className="memberTable">{members.map(m=><div className="memberRow" key={m.id}>{memberIdentity(m)}<select value={m.role} disabled={saving===m.id} onChange={e=>changeRole(m.id,e.target.value)}>{ASSIGNABLE_ROLES.map(([value,label])=><option key={value} value={value}>{label}</option>)}</select></div>)}</div></div>}
